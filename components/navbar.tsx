    ...
    <header className="sticky top-0 z-50 border-b border-brand-red/10 bg-brand-cream/75 backdrop-blur-xl dark:border-white/10 dark:bg-stone-950/70">
      <div className="section-shell flex h-20 items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-red text-lg font-bold text-white shadow-lg shadow-brand-red/20">
            A
          </div>
          <div>
            <p className="font-heading text-xl text-brand-red">AahaFoods</p>
            <p className="text-xs uppercase tracking-[0.3em] text-brand-ink/50 dark:text-stone-400">
              Homemade Luxury
            </p>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-2 lg:flex">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full px-4 py-2 text-sm font-medium text-brand-ink/80 transition hover:bg-white hover:text-brand-red dark:text-stone-200 dark:hover:bg-white/10"
            >
              {link.label}
            </Link>
          ))}
